...
namespace IFY.Phorm.Tests
{
    ...
    public class PhormContractRunnerTests
    {
        ...
        public void ManyAsync__Resolves_secondary_resultset()
        {
            ...
            // Act
            var res = runner.ManyAsync<TestParent>(new TestContract { Arg = 1 }).Result;

            // Assert
            Assert.AreEqual(3, res.Length);
            Assert.AreEqual(1, res[0].Children.Length);
            Assert.AreEqual("value1", res[0].Children[0].Value);
            Assert.AreEqual(0, res[1].Children.Length);
            Assert.AreEqual(0, res[2].Children.Length);
        }

        #endregion Additional resultsets
        ...
        public void One__Resolves_secondary_resultset()
        {
            ...
            // Act
            var res = runner.OneAsync<TestParent>(new TestContract { Arg = 1 }).Result;
